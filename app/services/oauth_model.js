...
function getClient(clientId, clientSecret) {
  ...
  return sqldb.OAuthClient
    ...
    .then(function (client) {
      ...
      // clientWithGrants.refreshTokenLifetime = 20
...
function getUser(username, password) {
  return User
    ...
    .then(function (user) {
      ...
      let salt = user.salt;
      var hashsalt = crypto.createHash(process.env.ALGORITHM_CRYTO).update(password+salt).digest('hex');
      console.log(hashsalt);
      return user.password == hashsalt ? user.toJSON() : false;
    })
    .catch(function (err) {
...
async function saveToken(token, client, user) {
  try {
    ...
    await OAuthAccessToken.create({
      ...
      token.refreshToken ? await OAuthRefreshToken.create({ // no refresh token for client_credentials
        refresh_token: token.refreshToken,
...
function validateScope(user, client, scope) {
  console.log(user.scope + "     " + client.scope + "  " + scope);
  return ((user.scope === client.scope) || (client.scope == scope)) ? client.scope : false
}
